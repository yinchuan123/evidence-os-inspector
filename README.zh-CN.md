...
1. **更正影响。** 五条主张、三个来源、两条已确认的依赖。来源 A 更正主要结果。工具根据记录的绑定和已确认依赖计算出 C1 与 C4 需复核，其余不受影响。
...
- **导出显式。** JSON 保留全部内容供继续工作；HTML 报告包含主张、绑定片段、状态和历史，默认不含来源全文。
...
你的文字在浏览器内处理。生产构建带有严格的 Content-Security-Policy，加载后禁止任何网络连接（`connect-src 'none'`）。导出只生成本地文件。托管页面的普通 Web 访问日志属于 GitHub Pages，与任何静态站点相同。
